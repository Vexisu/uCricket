...
\item maszyna wirtualna \ksremark{Musi być odpowiednie przypadek. Mamy przed wyliczeniem «… składającego się z:» zatem tutaj musi być «maszyny wirtualnej».} Java w wersji 17 lub wyższej;
\item zestaw \ksremark{jak wyżej} narzędzi avr-gcc wraz z linkerem i assemblerem.
\end{itemize}
...
\end{lstlisting}
Kompilator zwraca w pliku \lstinline|output.s| \ksremark{Czy da się wygenerować plik o innej nazwie?} program w języku assembly. Do zbudowania pliku binarnego, wykorzystywany jest \lstinline|avr-gcc|. Poniższe polecenie generuje plik \lstinline|output.bin| przeznaczony dla mikrokontrolera ATmega328P.
\begin{lstlisting}
...
\end{lstlisting}
\ksremark{Tak z ciekawości: to trzeba uruchamiać z sudo?}

\section{Język programowania}
\ksremark{zagajenie}
\subsection{Konwencja nazewnicza identyfikatorów}
Nazywanie elementów definiowanych przez użytkownika wymaga ścisłego używania nazw zgodnych z konwencją zapisu identyfikatorów. Identyfikatory przyjmują znaki alfanumeryczne, włącznie z dużymi i małymi literami, nie mogą zaczynać się cyfrą. \ksremark{Czy da się to opisać wyrażeniem regularnym?}
...
\ksremark{Wydaje mi się, że ten podrozdział byłoby lepiej przesunąć dalej.}
...
\end{lstlisting}
Katalogi w składni instrukcji \lstinline|import| oddzielane są dwukropkami \ksremark{Dlaczego nie można użyć `/'?} i muszą spełniać wymagania zapisu identyfikatorów. Nazwę pliku źródłowego należy przekazywać do instrukcji bez rozszerzenia, jest ono dodawane w trakcie skanowania plików źródłowych. 
...
\ksremark{Tab. \ref{tbl:typy-danych}: Tabele nie mają podpisu, ale „nadpis”.}
\ksremark{W nagłówki tab. \ref{tbl:typy-danych} wszystko małymi literami, tzn. «typ danych, słowa, definicje, opis».}
\begin{table}%[h!]
\begin{tabular}{@{}llll@{}}
\toprule
...
\end{table}
\subsection{Zmienne}
Zmienne mogą być definiowane jako globalne oraz lokalne w obszarze bloków funkcji, zmiennych i pętl \ksremark{pętli}. Typowanie zmiennych jest zawsze wymagane. Nie wymaga ono natomiast przypisania wartości. Definiowanie zmiennych odbywa się w następujący sposób:
\begin{lstlisting}
...
\end{lstlisting}
Pamięć alokowana przez zmienne jest zwalniana, gdy część wykonywana programu przesunie się poza klamry \comma\ np. po wyjściu z instrukcji warunkowej. Nie możliwe jest także odwoływanie się do zmiennych lokalnych wpisanych w klamry. \ksremark{Może lepiej to opisać jako zakresy zmiennych lokalnych.}
\begin{lstlisting}
if (true){
	var<Integer> i = 0;
}
...
i = i + 1;
\end{lstlisting}
\ksremark{Tutaj napisałbym jawnie, co się dzieje. Że zostaje zaalokowana pamięć na zmienną typu Ingeter i zapisany jej adres w zmiennej wskaźnikowej.}
...
\end{lstlisting}
\ksremark{Opisałbym to inaczej. Tutaj mamy dwa operatory dla wskaźników: = i $<-$. Jawnie bym to powiedział i opisał różnicę.}
...
\ksremark{Wszystko w tab. \ref{tbl:operatory-arytmetyczno-logiczne} małymi literami.}
\ksremark{Czy operatory w tab. \ref{tbl:operatory-arytmetyczno-logiczne} są spriorytetyzowane?}
...
\subsubsection{Pętle}
Dzięki pętlom możliwe jest wielorazowe wykonywanie kodu, dopóki warunek jest spełniany. Petlę definiuje się poprzez użycie słowa kluczowego \lstinline|while|, nawiasów okrągłych \ksremark{obejmujących warunek} i nawiasów klamrowych \ksremark{zawierających ciało pętli}. Tak samo jak w przypadku instrukcji warunkowej  warunek przekazywany do pętli musi zwracać dane typu boolowskiego.
\begin{lstlisting}
while (true) {
	// Code
}
...
\subsection{Funkcje}
W języku uCricket istnieje kilka kombinacji \ksremark{lepiej: rodzajów, sposobów} definicji funkcji:
\begin{itemize}
...
\end{lstlisting}
\ksremark{Czy istnieje przekazywanie wartości przez referencję?}
