...
\end{lstlisting}
\ksremark{A co z wyjściem? Co generuje kompilator?}

\section{Język programowania}
\ksremark{2-3 linijki zagajenia}
\subsection{Konwencja nazewniczna identyfikatorów}
Nazywanie elementów definiowanych przez użytkownika wymaga ścisłego używania nazw zgodnych z konwencją zapisu identyfikatorów. Identyfikatory przyjmują znaki alfanumeryczne, włącznie z dużymi i małymi literami, nie mogą zaczynać się cyfrą. \ksremark{Da się je opisać wyrażeniem regularnym?}
...
Pliki źródłowe mogą być łączone między sobą poprzez użycie słowa kluczowego \textit{import}:
\begin{lstlisting}
import katalog1:katalog2:NazwaPliku;
\end{lstlisting}
Katalogi w składni instrukcji \textit{import} oddzielane są dwukropkami i muszą spełniać wymagania zapisu identyfikatorów. Nazwę pliku źródłowego należy przekazywać do instrukcji bez rozszerzenia, jest ono dodawane w trakcie skanowania plików źródłowych. 

\subsection{Typy danych}
Język programowania wspiera kilka głównych typów danych wykorzystywanych w innych językach programowania. Typy danych muszą być ściśle definiowane w trakcie trakcie ich używania. W innym przypadku, kompilator przerwie proces kompilacji w trakcie analizy statycznej.
...
Język programowania uCricket implmeentuje wiele operatorów arytmetyczno-logicznych. Poniższa tabela opisuje zbór operatorów obsługiwanych przez język wraz z przyjmowanymi typami danych.
